...
async function runOnce(baseUrl) {
  const [meegleRes, githubRes] = await Promise.all([
    ...
    fetch(`${baseUrl}/api/integrations/github/issues`)
  ])
  ...
  const detail = `meegle=${meeglePayload?.detail || meeglePayload?.error || 'n/a'} | github_issues=${githubPayload?.detail || githubPayload?.error || 'n/a'}`
